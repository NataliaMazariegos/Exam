...
res.test.rd    =     rep(0,M)  #rd = ridge
res.train.rd   =     rep(0,M)
res.test.ls    =     rep(0,M)  #ls = lasso
res.train.ls   =     rep(0,M)
...
for (m in c(1:M)) {
  ...
  res.test.ls[m]   =     y.test - y.test.hat
  res.train.ls[m]  =     y.train - y.train.hat
  ...
  res.test.rd[m]   =     y.test - y.test.hat
  res.train.rd[m]  =     y.train - y.train.hat
  ...
  cat(sprintf("m=%3.f|  Rsq.test.en=%.2f, Rsq.test.ls = %.2f, Rsq.test.rd = %.2f, Rsq.test.rf=%.2f | 
              Rsq.train.en=%.2f, Rsq.train.ls=%.2f, Rsq.train.rd=%.2f, Rsq.train.rf=%.2f| \n", m,  
              Rsq.test.rf[m], Rsq.test.en[m], Rsq.test.ls[m],  Rsq.test.rd[m],  
              Rsq.train.rf[m], Rsq.train.en[m], Rsq.train.ls[m], Rsq.train.rd[m]))
  
  cat(sprintf("m=%3.f|  res.test.en=%.2f, res.test.ls = %.2f, res.test.rd = %.2f, res.test.rf=%.2f | 
              res.train.en=%.2f, res.train.ls=%.2f, res.train.rd=%.2f, res.train.rf=%.2f| \n", m,  
...
#las.cv.fit$lambda.min
#plot(glmnet(X.train, y.train, alpha=0))
#plot(lasso.fit)
...
boxplot(Rsq.test.en)
boxplot(Rsq.train.en)
boxplot(Rsq.test.ls)
boxplot(Rsq.train.ls)
boxplot(Rsq.test.rd)
boxplot(Rsq.train.rd)

#par(mfrow=c(8,2))
boxplot(res.test.rf)
boxplot(res.train.rf)
boxplot(res.test.en)
boxplot(res.train.en)
boxplot(res.test.ls)
boxplot(res.train.ls)
boxplot(res.test.rd)
boxplot(res.train.rd)
